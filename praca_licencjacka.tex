...
\usepackage{tabularx} % do wstawiania tabeli
\usepackage{amsfonts, amsmath, amsthm, amssymb} % dodatkowe symbole
\usepackage{pdfpages}
\usepackage{lipsum}
\usepackage{multirow}
\usepackage{caption}
...
\usepackage{tikz} % do rysowania grafów
\usepackage{algorithm} % wstawianie algorytmu
\usepackage{algpseudocode} % wstawianie algorytmu
\usepackage{listings}
\usepackage{xcolor}
\usepackage{epstopdf} % konwersja logo umcs z eps do pdf
\usepackage{pgf} % diagramy z matplotlib
\usepackage[hyphens]{url}
\usepackage[hidelinks]{hyperref}

% marginesy i interlinia
\usepackage{geometry}
\geometry{lmargin=3.5cm, rmargin=2.0cm, tmargin=1in, bmargin=1in}
\linespread{1.25}

% ustawienia do linków 
\hypersetup{breaklinks=true}
\urlstyle{same}

% wybierz figury używane z tikz
\usetikzlibrary{shapes.geometric, arrows, positioning, angles, quotes}
\graphicspath{ {./img/} }

% kształty tikz dla diagramu przepływu
...
% kształty tikz dla łańcuchów Markova
...
% zmień caption w środowisku algorithm
\makeatletter
...
\author{\textbf{Wiktor Wajszczuk}}
\title{Zastosowanie uczenia przez wzmacnianie w środowisku agentowym.}
\date{{\textbf{Lublin, rok \the\year}}}
\def\engtitle{Application of reinforcement learning in an agent\dywiz based
	      environment}
...
\def\rodzaj{licencjacka}
\def\jednostka{Katedrze Oprogramowania Systemów Informatycznych}
\def\promotor{dr Marcina Denkowskiego}
...
\multicolumn{2}{l}{\hspace{0mm}\includegraphics[scale=0.4]
					{UMCS_logotyp_gray}} \\
\multicolumn{2}{@{\hspace{16mm}}l}{\vspace{7mm}} \\

\multicolumn{2}{@{\hspace{1mm}}l}{\Large UNIWERSYTET MARII
					CURIE-SKŁODOWSKIEJ} \\
\multicolumn{2}{@{\hspace{1mm}}l}{\vspace{-4mm}} \\
\multicolumn{2}{@{\hspace{1mm}}l}{\Large W LUBLINIE} \\
\multicolumn{2}{@{\hspace{1mm}}l}{\vspace{-4mm}} \\
\multicolumn{2}{@{\hspace{1mm}}l}{\Large Wydział Matematyki, Fizyki i
					Informatyki} \\
\multicolumn{2}{@{\hspace{1mm}}l}{\vspace{16mm}} \\
...
& \vspace{-7mm} \\
& \multicolumn{1}{m{13cm}}{napisana w \jednostka} \\
& \vspace{-7mm} \\
& \multicolumn{1}{m{13cm}}{pod kierunkiem \promotor} \\
\multicolumn{2}{@{\hspace{0mm}}l}{\vspace{7mm}} \\
\multicolumn{2}{@{\hspace{0mm}}l}{\@date}
\end{tabular}
\end{table}
\end{titlepage}
}
...
\end{equation}
\begin{algorithm}
	\caption{Q-Learning \cite{ai_foundations_qlearning}}
\begin{algorithmic}[1]
...
\begin{figure}[!htb]
	\begin{center}
		\input{plots/plotFirstApproach.pgf}
...
\begin{figure}[!htb]
	\begin{center}
		\input{plots/plotTopPipeDeathPenalty.pgf}
...
\begin{figure}[!htb]
	\begin{center}
		\input{plots/plotGrid5x5TopPipePenalty.pgf}
