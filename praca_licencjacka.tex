...
Sam obiekt ptaka jest instancją klasy dziedziczącej po
...
po \textit{pygame.sprite.Sprite}. Generuję liczbę losową $x$ z zakresu
...
obrazek. Po wczytaniu obrazku w konstruktorze używam \textit{get\_rect}
będącej metodą klasy \textit{pygame.Surface}, której instancją jest nadany w
konstruktorze obrazek. Metoda ta mapuje obrazek na najmniejszy prostokąt, w
...
(\textit{sprite.Rect}) do określenia pozycji, na której ma zostać narysowany.
Rysowanie obiektów rur odbywa się w ten sam sposób, z tym, że wspomniana
metoda jest wywoływana na odpowiednio grupie obiektów rur dolnych i górnych.
Następnie na grupie, do której należy obiekt ptaka wywołuję metodę
\textit{update}, która obsługuje logikę ``latania''.

Sprawdzanie, czy zwiększyć punkt odbywa się w następujący sposób:
%napisz tutaj cos XD nie no na serio to ze jak przelatuje przez czarna linie
%jak ta linia jest ustalana
%i ze to jest potrzebne bo inaczej jakbysmy tylko ta czarne linie sprawdzali
%czy przelecial to moglby uderzyc od razu w pipa za nia i dalej bylby za to
%punkt
\begin{figure}
	\begin{center}
		\includegraphics{flappy_scoring}
		\caption{Przyznawanie punktów}
	\end{center}
	\label{flappy_scoring}
\end{figure}
