...
\begin{figure}[!htb] 
	\begin{center}
		\includegraphics[scale=0.39]{agent_top_pipe_penalty.png}
	\end{center}
...
jeżeli ostatnią akcją agenta było podskoczenie (\textit{self.last\_action}
...
$532 800$ elementów, co dalej stanowi około $\frac{1}{24}$ ilości elementów
...
\chap{Podsumowanie}
Przedstawiona w ninejszej pracy problematyka nie wyczerpuje w całości
zagadnienia uczenia przez wzmacnianie, jednak szczegółowo opisuje działanie
algorytm Q-Learning. Samodzielne zaimplementowanie gry ``Flappy Bird'' pomogło
mi w zrozumieniu i opisaniu jak agent działa w środowisku. Uważam, że wybrane
przezemnie technologie były trafionym pomysłem, były łatwe w użyciu i
...
tak zoptymalizować algorytm, by agent w ogóle nie mógl
przegrać\cite{tds_rl_score}. Myślę, że mógłym bardziej optymalizować swój
algorytm, jednak potrzebnaby była modyfikacja programu by ta nie przedstawiała
graficznej wizualizacji gry (wszak agent potrzebuje tylko informacji o
położeniu najbliższego obiektu rury, nie potrzeba wizualizacji). Wtedy mógłbym
zaimplementować tak główną pętlę programową by ta nie wykonywała się raz na
tyknięcie symulowanego zegara, a tak szybko jak pozwalałby na to procesor, na
którym program został uruchomiony. Niestety do dalszych optymalizacji zabrakło
mi czasu.
	
Ninejsza praca pokazuje jak skomplikowanym i fascynującym zagadnieniem może
...
nakład pracy by osiągać naprawde dobre wyniki.
