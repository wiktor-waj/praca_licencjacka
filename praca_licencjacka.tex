...
% YYY popraw ogólniki na konkretne numery rozdziałów
...
\chapter{Gra}
Problem, który postawiłem przed uczeniem przez wzmacnianie to nauczenie się
grania w ``Flappy Bird'', grę z 2013 autorstwa wietnamskiego developera Dong
Nguyena\cite{flappy_bird_author}. Wybrałem tę grę ze względu na ograniczoną
ilość akcji jaką może podjąć gracz.
% YYY tu skończyłeś
\section{Zasady gry} 
\begin{figure}[h] 
	\begin{center}
...
\end{split}
\end{equation}
W dalszej części pracy będę rozważał nagrodę skumluowaną jako nagrodę
dyskontową.
...
polityka $\pi'$ dla dowlonego stanu $s$, że $V^{\pi'}(s) > V^{\pi}(s)$. To
...
Agent może użyć równania na różnicę czasową \ref{td_3} by zaktualizować swoje
...
\[\forall x \in [-80, 509] \cap \mathbb{Z} \ \forall y \in [-300,799] \cap
\mathbb{Z} \ \forall v \in [-10,8] \cap \mathbb{Z}\ \  \{x, y, v\}\]
...
algorytm, jednak potrzebna by była modyfikacja programu by ten nie przedstawiał
